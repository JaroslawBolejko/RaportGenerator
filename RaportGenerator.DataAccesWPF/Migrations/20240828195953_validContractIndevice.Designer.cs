// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using RaportGenerator.DataAccess;

#nullable disable

namespace RaportGenerator.DataAccess.Migrations
{
    [DbContext(typeof(RaportGeneratorContext))]
    [Migration("20240828195953_validContractIndevice")]
    partial class validContractIndevice
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "7.0.0")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("RaportGenerator.DataAccess.Entities.Company", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ApartmentNumber")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("BankAccountNumber")
                        .HasMaxLength(28)
                        .HasColumnType("nvarchar(28)");

                    b.Property<string>("City")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("CompanyName")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<int>("CompanyRole")
                        .HasColumnType("int");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("Number")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("Street")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("TaxNumber")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("TelefonNumber")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<string>("ZipCode")
                        .IsRequired()
                        .HasMaxLength(6)
                        .HasColumnType("nvarchar(6)");

                    b.HasKey("Id");

                    b.ToTable("Companies");
                });

            modelBuilder.Entity("RaportGenerator.DataAccess.Entities.Contract", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int?>("CompanyId")
                        .IsRequired()
                        .HasColumnType("int");

                    b.Property<DateTime>("ContractDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsValid")
                        .HasColumnType("bit");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Number")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Term")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("CompanyId");

                    b.ToTable("Contracts");
                });

            modelBuilder.Entity("RaportGenerator.DataAccess.Entities.ContractDevice", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("ContractId")
                        .HasColumnType("int");

                    b.Property<int>("DeviceId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("ContractId");

                    b.HasIndex("DeviceId");

                    b.ToTable("ContractDevices");
                });

            modelBuilder.Entity("RaportGenerator.DataAccess.Entities.Device", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("BlackWhitePrintCounter")
                        .HasColumnType("int");

                    b.Property<int>("ColorPrintCounter")
                        .HasColumnType("int");

                    b.Property<string>("DeviceName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("SerialNumber")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("ValidContractId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("ValidContractId");

                    b.ToTable("Devices");
                });

            modelBuilder.Entity("RaportGenerator.DataAccess.Entities.Report", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("BlackPrintsAmount")
                        .HasColumnType("int");

                    b.Property<int>("ColorPrintsAmount")
                        .HasColumnType("int");

                    b.Property<int?>("CompanyId")
                        .IsRequired()
                        .HasColumnType("int");

                    b.Property<int?>("ContractId")
                        .HasColumnType("int");

                    b.Property<int>("CurrentPrintsStateBlack")
                        .HasColumnType("int");

                    b.Property<int>("CurrentPrintsStateColor")
                        .HasColumnType("int");

                    b.Property<DateTime>("DateOfDokument")
                        .HasColumnType("date");

                    b.Property<DateTime>("DateOfService")
                        .HasColumnType("date");

                    b.Property<int?>("DeviceId")
                        .IsRequired()
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("Number")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("PrevPrintsStateBlack")
                        .HasColumnType("int");

                    b.Property<int>("PrevPrintsStateColor")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("CompanyId");

                    b.HasIndex("ContractId");

                    b.HasIndex("DeviceId");

                    b.ToTable("Reports");
                });

            modelBuilder.Entity("RaportGenerator.DataAccess.Entities.Contract", b =>
                {
                    b.HasOne("RaportGenerator.DataAccess.Entities.Company", "Company")
                        .WithMany("Contracts")
                        .HasForeignKey("CompanyId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Company");
                });

            modelBuilder.Entity("RaportGenerator.DataAccess.Entities.ContractDevice", b =>
                {
                    b.HasOne("RaportGenerator.DataAccess.Entities.Contract", "Contract")
                        .WithMany("ContractDevices")
                        .HasForeignKey("ContractId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("RaportGenerator.DataAccess.Entities.Device", "Device")
                        .WithMany("ContractDevices")
                        .HasForeignKey("DeviceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Contract");

                    b.Navigation("Device");
                });

            modelBuilder.Entity("RaportGenerator.DataAccess.Entities.Device", b =>
                {
                    b.HasOne("RaportGenerator.DataAccess.Entities.Contract", "ValidContract")
                        .WithMany("Devices")
                        .HasForeignKey("ValidContractId");

                    b.Navigation("ValidContract");
                });

            modelBuilder.Entity("RaportGenerator.DataAccess.Entities.Report", b =>
                {
                    b.HasOne("RaportGenerator.DataAccess.Entities.Company", "Company")
                        .WithMany("Reports")
                        .HasForeignKey("CompanyId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("RaportGenerator.DataAccess.Entities.Contract", null)
                        .WithMany("Reports")
                        .HasForeignKey("ContractId");

                    b.HasOne("RaportGenerator.DataAccess.Entities.Device", "Device")
                        .WithMany()
                        .HasForeignKey("DeviceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Company");

                    b.Navigation("Device");
                });

            modelBuilder.Entity("RaportGenerator.DataAccess.Entities.Company", b =>
                {
                    b.Navigation("Contracts");

                    b.Navigation("Reports");
                });

            modelBuilder.Entity("RaportGenerator.DataAccess.Entities.Contract", b =>
                {
                    b.Navigation("ContractDevices");

                    b.Navigation("Devices");

                    b.Navigation("Reports");
                });

            modelBuilder.Entity("RaportGenerator.DataAccess.Entities.Device", b =>
                {
                    b.Navigation("ContractDevices");
                });
#pragma warning restore 612, 618
        }
    }
}
